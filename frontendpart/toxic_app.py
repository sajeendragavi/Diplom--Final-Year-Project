...
def predict():
    ...
    return render_template('index_toxic.html', 
                            pred_tox = 'Prob (токсический/Toxic): {}'.format(out_tox),
                            pred_sev = 'Prob (Сильно токсичен/Severe Toxic): {}'.format(out_sev), 
                            pred_obs = 'Prob (Непристойный/Obscene): {}'.format(out_obs),
                            pred_ins = 'Prob (Оскорблять/Insult): {}'.format(out_ins),
                            pred_thr = 'Prob (Угроза/Threat): {}'.format(out_thr),
                            pred_ide = 'Prob (Личность Ненависть/Identity Hate): {}'.format(out_ide)                        
                            )
